...
def train(pre_epochs):
    ...
    if pre_epochs > 0:
        pre_model = PreBert()
        # 对每个样本进行采样，拼成句子。格式{'data': data, 'label': label, 'paths': paths}
        # data：句子，sample_num条四元组拼成的句子；
        # label: 最后一条句子的客体
        # paths: 用到的所有四元组
        pre_model.to(device)
        ...
        data_path = './data/{}/bert_pre_train.pkl'.format(dataset)
        bert_pre_train = md.get_data('train')
        if not os.path.isfile(data_path):
            with open(data_path, 'wb') as f:
                pickle.dump(bert_pre_train, f)
        ...
        for epoch in range(pre_epochs):
            ...
            for step, batch in loop:
                ...
                # 哪些是真实token，哪些是填充的token
                attention_mask = batch['attention_mask'].to(device)
                # inuts_ids的copy，记录标签信息
...
def test():
    ...
    print('now is {}'.format(args.m))
    bert_test_batchs = md.get_test_data(args.m)
    hitsRank = [1, 3, 10]
    mrrCount = 0
    ...
    with torch.no_grad():
        ...
        print('\t'.join(results))
        metric = ['mrr', 'hit@1', 'hit@3', 'hit@10']
        results_dict = {metric[i]: results[i] for i in range(len(metric))}
        save_path = './bert/bert_base_cased_{}/pretrain_{}/epoch_{}/results.json'.format(dataset, mi, ep)
